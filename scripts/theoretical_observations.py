...
def compute_omega(codon_frequencies_seq, aa_fitness_seq, mutation_matrix):
    d, d0 = 0, 0
    dsw, dsw0 = 0, 0
    dws, dws0 = 0, 0
    ...
    for site in range(n):
        ...
        for x in range(len(codons)):
            d_partiel, d0_partiel = 0.0, 0.0
            dsw_partiel, dsw0_partiel = 0.0, 0.0
            dws_partiel, dws0_partiel = 0.0, 0.0
            ...
            for y, a, b in non_syn_neighbors[x]:
                ...
                d_partiel += mutation_matrix[a][b] * p_fix
                d0_partiel += mutation_matrix[a][b]

                if (nucleotides[a] in strong_nucleotides) and (nucleotides[b] in weak_nucleotides):
                    dsw_partiel += mutation_matrix[a][b] * p_fix
                    dsw0_partiel += mutation_matrix[a][b]

                if (nucleotides[a] in weak_nucleotides) and (nucleotides[b] in strong_nucleotides):
                    dws_partiel += mutation_matrix[a][b] * p_fix
                    dws0_partiel += mutation_matrix[a][b]

            d += codon_frequencies_site[x] * d_partiel
            d0 += codon_frequencies_site[x] * d0_partiel

            dsw += codon_frequencies_site[x] * dsw_partiel
            dsw0 += codon_frequencies_site[x] * dsw0_partiel

            dws += codon_frequencies_site[x] * dws_partiel
            dws0 += codon_frequencies_site[x] * dws0_partiel

    omega = d / d0
    return omega, (dsw / dsw0) / omega, (dws / dws0) / omega


def generate_frequencies(alpha_param, lambda_param, n):
    ...
    omega, omegasw, omegaws = compute_omega(codon_frequencies_seq, aa_fitness_seq, mutation_matrix)
...
def plot_heatmap(nbr_steps, n):
    ...
    for x_i, lambda_param in enumerate(x_axis):
        for y_i, alpha_param in enumerate(y_axis):
            stat_list = generate_frequencies(alpha_param, lambda_param, n)
    ...
    for index, z in enumerate(heatmap_summary_stats):
        ...
        for dimensions in ["2d", "3d"]:
            ...
            if dimensions == "3d":
                ...
                surf = ax.plot_surface(logx, logy, zommed_z, cmap='RdBu', linewidth=0, antialiased=True)

                ax.set_zlim(zommed_z.min() - 0.01, zommed_z.max() + 0.01)
                ...
                ax.set_ylabel('log($\\alpha$)', fontsize=12)
                ax.set_zlabel(stat_names[index], fontsize=12)
            else:
            ...
            plt.savefig("../figures/{0}_{1}.svg".format(stat_title[index], dimensions), format="svg")
            plt.savefig("../figures/{0}_{1}.png".format(stat_title[index], dimensions), format="png")
            plt.clf()
            plt.close('all')


plot_heatmap(30, 5000)
